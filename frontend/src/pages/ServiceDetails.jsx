    ...
    <div className="min-h-screen bg-slate-50 flex flex-col font-sans">
      ...
      <div className="bg-slate-900 text-white py-16 relative overflow-hidden">
        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <Link to="/services" className="inline-flex items-center text-blue-400 hover:text-white mb-6 font-medium transition-colors">
            <ChevronLeft className="w-4 h-4 mr-1" /> Back to all services
          </Link>
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
            <div>
              {!service.active && (
                <span className="inline-block px-3 py-1 bg-white/10 backdrop-blur-sm text-white rounded-full text-xs font-bold uppercase tracking-wider mb-4 border border-white/20">
                  Coming Soon
                </span>
              )}
            ...
            <div className="text-left md:text-right">
              <p className="text-slate-400 text-sm uppercase tracking-wide font-bold mb-1">Starting from</p>
              <p className="text-5xl font-extrabold text-white">₹{service.price}</p>
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
          ...
          <div className="lg:col-span-1">
            <div className="bg-white rounded-2xl shadow-lg border border-slate-200 p-6 sticky top-28">
              <h3 className="text-xl font-bold text-slate-900 mb-2">Ready for a wash?</h3>
              <p className="text-slate-600 mb-6">Book this service now and our professionals will be at your doorstep.</p>
              
              {service.active ? (
                <button 
                  onClick={() => navigate('/book', { state: { serviceId: service.id } })}
                  className="w-full py-4 bg-blue-600 text-white rounded-xl font-bold text-lg hover:bg-blue-700 transition-colors shadow-md hover:shadow-lg flex justify-center items-center"
                >
                  Book Now
                </button>
              ) : (
                <button disabled className="w-full py-4 bg-slate-100 text-slate-500 rounded-xl font-bold text-lg cursor-not-allowed">
                  Currently Unavailable
                </button>
              )}
              
              <p className="text-xs text-center text-slate-500 mt-4">No credit card required. Pay after service.</p>
            </div>
          </div>
